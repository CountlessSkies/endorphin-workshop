...
function parseProject(value) {
    try {
        ...
        return { root_folder: String(data?.root_folder || DEFAULT_PROJECT.root_folder), project_id: String(data?.project_id || (workflow === "artwork" ? "2608001" : "RD2608001")), workflow_type: workflow, source_type: data?.source_type === "print_reference" ? "print_reference" : "embroidery_reference", route: String(data?.route || (workflow === "artwork" ? "artwork_foundation" : data?.source_type === "print_reference" ? "redesign_print_candidate" : "redesign_emb_candidate")), mockup_variant: data?.mockup_variant === "emb" ? "emb" : "print", candidate_letter: /^[A-Z]+$/i.test(String(data?.candidate_letter || "")) ? String(data.candidate_letter).toUpperCase() : "", palette: normalizePalette(data?.palette), creation_period: /^\d{4}$/.test(String(data?.creation_period || "")) ? String(data.creation_period) : DEFAULT_PROJECT.creation_period };
...
function createProjectSelector(node, inputName, inputData) {
    ...
    const contentMinHeight = () => {
        ...
        if (["artwork_foundation", "artwork_mockup", "artwork_stitchwork"].includes(project.route)) return height + (stagePreviewHeight + 39) * 2;
    ...
    async function refreshProjects() {
        ...
        try { const response = await fetch(`/endorphin/etsy/projects?${new URLSearchParams({ root_folder: project.root_folder, workflow_type: project.workflow_type, period: project.creation_period })}`); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Could not scan project folders."); projects = Array.isArray(data.projects) ? data.projects : []; if (project.project_id && !projects.includes(project.project_id)) { project.project_id = ""; project.candidate_letter = ""; commit(); } status = projects.length ? `${projects.length} existing ${project.creation_period} ID${projects.length === 1 ? "" : "s"}.` : `No existing IDs for ${project.creation_period}.`; } catch (error) { status = error.message || "Could not scan project folders."; projects = []; } finally { loading = false; render(); resize(); }
    }
    ...
    function stepProject(delta) {
        ...
        project.project_id = projects[index]; project.candidate_letter = ""; commit(); refreshCandidates(); render();
    }
    ...
    function renderStagePreview(previewKind = "input") {
        const names = { artwork_foundation: "artwork_<ID>_transparent", artwork_mockup: "base_<ID>_transparent", artwork_stitchwork: "mockup_<ID>_print", artwork_colorway: "mockup_<ID>_emb", redesign_emb_candidate: "source reference", redesign_print_candidate: "source reference" };
        const titles = { artwork_foundation: "Foundation input preview", artwork_mockup: "Mockup placement input preview", artwork_stitchwork: "Stitchwork input preview", artwork_colorway: "Colorway input preview", redesign_emb_candidate: "Candidate input preview", redesign_print_candidate: "Candidate input preview" };
        const outputNames = { artwork_foundation: "base_<ID>_transparent", artwork_mockup: `mockup_<ID>_${project.mockup_variant}`, artwork_stitchwork: "mockup_<ID>_emb" };
    ...
    function renderCandidateSelector(allowApproval = false) {
        ...
        const heading = document.createElement("div"); heading.style.cssText = "display:flex;align-items:center;justify-content:space-between;gap:6px;";
        heading.append(label(allowApproval ? "Redesign candidate" : "Generated candidates preview"));
        const refresh = button("↻", refreshCandidates, loadingCandidates); refresh.title = "Refresh candidates from folder"; refresh.style.cssText += "width:28px;padding:0;margin-top:6px;";
        heading.append(refresh); section.append(heading);
    ...
    function render() {
        ...
        root.append(label("Project date filter (+ New)")); const dateRow = document.createElement("div"); dateRow.style.cssText = "display:grid;grid-template-columns:1fr 1fr;gap:5px;"; const yearSelect = document.createElement("select"); yearSelect.style.cssText = inputCss; const selectedYear = Number(`20${project.creation_period.slice(0, 2)}`), years = creationYears(); if (!years.includes(selectedYear)) years.push(selectedYear); for (const year of years.sort((a, b) => a - b)) { const option = document.createElement("option"); option.value = String(year).slice(-2); option.textContent = `Year ${year}`; yearSelect.append(option); } yearSelect.value = project.creation_period.slice(0, 2); const monthSelect = document.createElement("select"); monthSelect.style.cssText = inputCss; for (let month = 1; month <= 12; month += 1) { const option = document.createElement("option"); option.value = String(month).padStart(2, "0"); option.textContent = `Month ${String(month).padStart(2, "0")}`; monthSelect.append(option); } monthSelect.value = project.creation_period.slice(2, 4); const commitPeriod = () => { project.creation_period = `${yearSelect.value}${monthSelect.value}`; project.project_id = ""; project.candidate_letter = ""; commit(); refreshProjects(); }; yearSelect.onchange = commitPeriod; monthSelect.onchange = commitPeriod; stopCanvasEvents(yearSelect); stopCanvasEvents(monthSelect); dateRow.append(yearSelect, monthSelect); root.append(dateRow);
        root.append(label("ID")); const idRow = document.createElement("div"); idRow.style.cssText = "display:grid;grid-template-columns:minmax(150px,1fr) 28px 28px auto auto;gap:5px;"; const select = document.createElement("select"); select.style.cssText = inputCss; const placeholder = document.createElement("option"); placeholder.value = ""; placeholder.textContent = loading ? "Loading IDs…" : "Select existing ID…"; select.append(placeholder); for (const id of projects) { const option = document.createElement("option"); option.value = id; option.textContent = id; select.append(option); } select.value = project.project_id || ""; select.onchange = () => { project.project_id = select.value; project.candidate_letter = ""; commit(); refreshCandidates(); }; stopCanvasEvents(select); const previous = button("↑", () => stepProject(-1), loading || !projects.length); const next = button("↓", () => stepProject(1), loading || !projects.length); previous.style.cssText += "width:28px;padding:0;"; next.style.cssText += "width:28px;padding:0;"; idRow.append(select, previous, next, button("↻", refreshProjects, loading), button("+ New", createProject, loading)); root.append(idRow);
        ...
        if (project.workflow_type === "artwork") root.append(cards([["artwork_foundation", "Foundation"], ["artwork_mockup", "Mockup"], ["artwork_stitchwork", "Stitchwork"], ["artwork_colorway", "Colorway"]], project.route, (value) => { project.route = value; project.source_type = "idea_artwork"; }));
        ...
        if (project.route === "artwork_mockup") { root.append(label("Mockup output")); root.append(cards([["print", "Print"], ["emb", "Embroidery"]], project.mockup_variant, (value) => { project.mockup_variant = value; })); }
        if (["artwork_foundation", "artwork_mockup", "artwork_stitchwork", "artwork_colorway", "redesign_emb_candidate", "redesign_print_candidate"].includes(project.route)) root.append(renderStagePreview());
        if (["artwork_foundation", "artwork_mockup", "artwork_stitchwork"].includes(project.route)) root.append(renderStagePreview("output"));
